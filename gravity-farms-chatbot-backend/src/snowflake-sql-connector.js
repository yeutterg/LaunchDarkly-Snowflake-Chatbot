...
class SnowflakeSQLConnector {
    constructor() {
        ...
        if (this.isDemo) {
            console.log('🎮 Running in DEMO MODE - Using mock data instead of Snowflake SQL functions');
        } else {
            console.log('🚀 Running in PRODUCTION MODE - Using Snowflake SQL functions and LaunchDarkly AI Configs');
            console.log('✅ Cortex SQL functions confirmed available in your account');
        }
    }

    async snowflakeSQLClient(sqlQuery) {
        const headers = {
    ...
    async generateResponse(userMessage, context, sessionId) {
        ...
        try {
            ...
            console.log('🚀 Making Snowflake SQL call with:');
            console.log('   Model:', config.model.name);
            console.log('   Messages count:', config.messages.length);
            console.log('   SQL Query:', sqlQuery);
            
            const run = await this.snowflakeSQLClient(sqlQuery);

            console.log('📡 Snowflake SQL response status:', run.status);
            console.log('📡 Snowflake SQL response headers:', Object.fromEntries(run.headers.entries()));
            ...
            try {
                ...
                throw new Error('Invalid JSON response from Snowflake SQL API');
            }
            ...
            const response = result.data?.[0]?.[0] ?? "No response from Snowflake SQL";
    ...
    async searchProducts(searchQuery, limit = 5) {
        ...
        console.log('Product search not implemented for SQL interface yet');
        return [];
    }
    ...
    async lookupOrder(customerEmail, orderId = null) {
        ...
        // For production, you would implement SQL queries to your Snowflake data
        console.log('Order lookup not implemented for SQL interface yet');
        return [];
    }
}

module.exports = { SnowflakeSQLConnector }; 